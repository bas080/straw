...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var search_default = register;

  // state.mjs
  var state = (initial) => {
    let registered = [];
    let oldState = initial();
    return function register2(onState2) {
      const internalOnState = (state2) => onState2(state2, pushState);
  ...
  var onChange = (hasChanged = notEquals) => {
  ...
  function search(onState2) {
    ...
    onState2((state2, push) => {
      ...
      index.forEach(([elem, content]) => {
        if (isEmpty(state2.tokens) || fuzzy(content, search)) {
  ...
  onState((state2) => {
    if (state2.query)
      updateQueryParam("q", state2.query);
    else
      deleteQueryParam("q");
